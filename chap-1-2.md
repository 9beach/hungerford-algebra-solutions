...
**_Proof._**&nbsp;$\quad$Let $a,b\in G$; let $f$ be
the map. Remind that by Exercise 1-11 (iii),
$G$ is abelian $\Leftrightarrow (ab)^{-1}=a^{-1}b^{-1}$.
_Case $\Rightarrow$._ If $G$ is abelian,
then $f(ab)=(ab)^{-1}=a^{-1}b^{-1}=f(a)f(b)$. Thus $f$ is a homomorphism.
There is $a^{-1}$ such that $f(a^{-1})$ = $a$ for every $a$; thus
surjective. If $f(a)\neq f(b)$, then $a^{-1}\neq b^{-1}\Leftrightarrow
e\neq b^{-1}a$ $\Leftrightarrow$ $a\neq b$; thus injective. Therefore,
...
\end{pmatrix}$ $\neq$ $\begin{pmatrix}
 -i &  0\\
 \phantom{-}0 & i
\end{pmatrix}$ $=BA$; thus non-abelian. $BA=-AB=-AIB=A^3B$.
Notice that by $BA=A^3B$ and induction, we have $B^mA^n=A^pB^m$
for some $p$. Thus every element of $Q_8$ is of the form $A^iB^j$.
From this, we have that $Q_8=\{I,A,A^2,A^3,B,AB,A^2B,A^3B\}$.$\quad\square$
...
Suppose that there is an isomorphism $g : Q_8 \to H$.
Let $S=\{A, -A, B, A^2B,AB, A^3B\}$. Then for every $X\in S$, $X^4=I$ and
$X^2\neq I$, thus the order of $g(X)$ is 6, and for every $Y\in g(X)$,
$Y^2\neq I$ and $Y^4=I$. But $H$ has only 2 elements, $C$ and $C^3$ with the
...
$a\in S$ $\Rightarrow$ $a=ae^{-1}\in S$ $\Rightarrow a\sim e\Rightarrow
e\sim a$. If $a,b\in S$, then $e\sim a$, $e\sim b\Rightarrow$
$a\sim b\Rightarrow ab^{-1}\in S$. By Theorem 2.5, $S$ is a subgroup.
\
&nbsp;$\quad$_Case $\Leftarrow$._ Suppose that $S$ is a subgroup of $G$.
For all $a\in G$, $aa^{-1}=e\in S$ $\Rightarrow a\sim a$. If $a\sim b$, then
...
If $a\in f^{-1}(B)$,
then $f(a)\in B \Rightarrow f(a)^{-1}=f(a^{-1})\in B \Rightarrow a^{-1}\in
f^{-1}(B)$. $f^{-1}(B)$ is a subgroup of $G$.
\
&nbsp;&nbsp;$\quad$(b) $g_H\in f(A)$. There is $a\in A$ such that $f(a)=b$
for every $b\in f(A)$, thus $f(a^{-1})=f(a)^{-1}=b^{-1}$. Similarly
there is $a,b\in A$ such that $f(a)=c, f(b)=d$ for every $c,d\in f(A)$, thus
$cd=f(a)f(b)\in f(A)$.
$\quad\square$
...
Similarly to Exercise 4, $(0, 1), (1, 0)\in {Z}_2
\bigoplus {Z}_2$ have order 2, but only $(2)\in Z_4$ has order 2.
Therefore, not isomorphic.$\quad\square$
...
**_Proof._**&nbsp;$\quad$Let $x\in G$. $ax=xa
\Rightarrow axa^{-1}=x$ $\Rightarrow a^{-1}axa^{-1}=a^{-1}x \Rightarrow
xa^{-1}=a^{-1}x$. $e_G\in C$. $ab=ba$ for every $a,b\in C$; thus abelian.
If $a,b\in C$, then $xab=axb=abx$, thus $ab\in C$.$\quad\square$
...
**_Proof._**&nbsp;$\quad$They are all cyclic groups of order 4. Map between
$i, 1$ and $\left< \begin{pmatrix}
 1 & 2 & 3 & 4\\
 2 & 3 & 4 & 1
...
$\{((0,0), 1_{Z_8}), ((0,1), 3_{Z_8}),$ $((1,0),5_{Z_8}), ((1,1),7_{Z_8})\}$
...
$\{\overline{1/p^n}\mid n\in \mathbb{N}^\ast\}$.
Since is clear that $S\subset \mathbb{Z}(p^\infty)$, we only show that
$S\supset \mathbb{Z}(p^\infty)$. Let $a\in\mathbb{Z}(p^\infty)$. Then
...
$H\cup K\subset S$ $\subset H \vee K$, we show that $S$ is a group.
$e_G\in S$. If $ab, cd\in S$ for some $a,b\in H$ and
$c,d\in K$, then $ab\in H$ and $cd\in K$, thus $abcd\in S$; closed under
the operation of $G$. If $ab\in S$ for some $a\in H$ and $b\in K$,
...
of an abelian group $G$. It is easily seen by induction that the join
...
subgroup of $G$. $\bigcap_{i \in I}H_i\subset t\}$. Since $G\in T$,
