...
$x\mapsto e$, and let $-f$ be a function in $M(S,G)$
given by $x\mapsto f(x)^{-1}$. It is clear that $0_S$ is an identity
...
**_Proof._**&nbsp;$\quad$Let $S$ be a set such that $|S|>1$, and
define $xy=y$ for all $x,y\in S$. $(xy)z=x(yz)=z$
for all $x,y,z\in S$; thus a semigroup. Pick any $a\in S$. Since
$ax=x$ for all $x\in S$, $a$ is a left identity element, and also
since $xa=a$ for all $x$, every $x$ has a right inverse
...
**_Proof._**&nbsp;$\quad$Let $a_1, a_2, b_1$, and $b_2$ be integers.
If $\overline{a}_1=\overline{a}_2$ and
$\overline{b}_1=\overline{b}_2$, then $a_1 = a_2+mp$ and
...
If $\overline{a}_1=\overline{a}_2$ and
$\overline{b}_1=\overline{b}_2$, then $a_1-a_2=m$ and
$b_1-b_2=n$ for some $m$ and $n\in\mathbb{Z}$. $(a_1+b_1)-(b_2+b_2)$ $=$
$m+n$. Thus $a_1+b_1\sim a_2+b_2$.
\
&nbsp;$\quad$(b) Let $m$ and $n$ be positive integers,
and let $k$ be an integer.
By Theorem 1.5, the set forms an abelian group. If
$1/m=1/n+k$, then $k=0$, thus $m=n$, and so
...
**_Proof._**&nbsp;$\quad$Let $x,y,z,a/b$, and $c/d\in R_p$. Since
...
$a/b+(-a/b)=0$. $x+y=y+x$.
\
&nbsp;$\quad$Let $a/b$ and $c/d\in R^p$. Since $a/b+c/d=(ad+bc)/bd$,
...
$\overline{0}=\overline{0/p}$, and that there is an inverse element
...
$ab(a^{-1}b^{-1})=aba^{-1}b^{-1}=aa^{-1}bb^{-1}=e$.
\
...
$\Leftrightarrow$ $ab^n=b^na$. By replacing $a$ and $b$ with $b$ and $a$,
...
for $n=0$ and $n=1$ in all groups. Thus (v) $\Rightarrow$ (i) is false.$\quad\square$
...
**_Proof._**&nbsp;$\quad$If $a,b\in G$, then
...
**_Proof._**&nbsp;$\quad$Let $a\in G$. Notice that if $a^2=e$ then $a=a^{-1}$.
...
$a=a^{-1}$, then
$\bigcup S=G \smallsetminus\{e\}$ has an even order; thus $G$ has
an odd order. Therefore, if $G$ is finite and $G$ has an even order,
...
for some $m$ and $n$ such that $1<m<n$. Since the operation is
associative, $aa^{m-1}=aa^{n-m+1}a^{m-1}$, and
using the right cancelation law, $a=aa^{n-m+1}$. Similarly,
$a=a^{n-m+1}a$. Thus $a=a^{k_a}a=aa^{k_a}$ for some $k_a>1$.
$ab=aa^{k_a}b$, thus $b=a^{k_a}b$. $ba=ba^{k_a}a$,
thus $b=ba^{k_a}$. $a^{k_a}$ is an identity element of $G$,
...
**_Proof._**&nbsp;$\quad$$f_n$ is well-defined for each
...
$n\in\mathbb{N}$. $\varphi(0)=a_1,\varphi(1)=a_1a_2,
